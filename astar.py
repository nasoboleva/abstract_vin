...
def _get_valid_neighbours(example, current, closed, dim=3, if_ours=False):
    if dim == 3:
    ...
    for x,y in [[-1,0],[0,-1],[0,1],[1,0]]:
        if dim == 2:
            ...
            if if_ours:
                if not 0 <= neighbour[0] < example.size or not 0 <= neighbour[1] < example.size:
                   continue
            # skip closed poses
...
def dijkstra_planner_2d(example, if_ours=False):
    start = (example.start[0],example.start[1])
    g_values = np.full((example.size, example.size), float('Inf'))
    g_values[start[0],start[1]]=0
    ...
    while open_list:
        ...
        neighbours = _get_valid_neighbours(example, current, closed, dim=2, if_ours=if_ours)
        for neighbour in neighbours:
